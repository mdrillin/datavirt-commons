...
/**
 * This is a simple servlet that generates the javascript data used by the DataVirt Header javascript.
 *
 * @author mdrillin@redhat.com
 */
public class DataVirtHeaderDataJS extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        if (appId == null || appId.trim().length() == 0) {
            throw new ServletException("Application identifier (app-id) parameter missing from DataVirt Header Data JS servlet.");
        }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
        ...
        try {
            ...
            response.getOutputStream().write("var DATAVIRT_HEADER_DATA = ".getBytes("UTF-8"));
...
// No Primary App Tabs for DataVirt UI.  May need to add back later
...
//            g.writeEndArray();
    ...
    private List<TabInfo> getTabs(HttpServletRequest request) throws Exception {
        ...
        List<TabInfo> tabs = (List<TabInfo>) session.getAttribute("datavirt-tabs");
        if (tabs == null) {
            tabs = new ArrayList<TabInfo>();
            getConfiguredTabs(tabs);
            session.setAttribute("datavirt-tabs", tabs);
        }
        return tabs;
    }
    ...
    private void getConfiguredTabs(List<TabInfo> tabs) throws Exception {
        ...
        for (File configFile : configFiles) {
            if (!configFile.getCanonicalPath().endsWith("-datavirtapp.properties"))
                continue;
            FileReader reader = new FileReader(configFile);
            try {
                Properties configProps = new Properties();
                configProps.load(new FileReader(configFile));
                String appId = configProps.getProperty("datavirtapp.app-id");
                String href = configProps.getProperty("datavirtapp.href");
                // TODO need i18n support here - need different versions of the config files for each lang?
                String primaryBrand = configProps.getProperty("datavirtapp.primary-brand");
                String secondaryBrand = configProps.getProperty("datavirtapp.secondary-brand");
                String label = configProps.getProperty("datavirtapp.label");
    ...
    /**
     * Returns the directory to check for datavirt-app config files.
     */
    private File getConfigDir() {
        File configDir = null;

        // First, check for a configured system property
        String configDirProp = System.getProperty("org.jboss.datavirt.apps.config-dir");
        ...
        if (jbossConfigDir != null) {
            ...
            if (dirFile.isDirectory()) {
                configDir = new File(dirFile, "datavirt-apps");
                if (configDir.isDirectory()) {
                    return configDir;
                }
            }
